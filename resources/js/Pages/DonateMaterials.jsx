...
import { CheckCircle, Search, Grid, List, Upload, SlidersHorizontal, ChevronDown, ChevronUp, Trash2 } from "lucide-react";
...
export default function Materials() {
  ...
  // NEW: state for collapsible filters
  ...
  const filteredMaterials = materials
    ...
    .filter((m) => {
      ...
      const matchesCondition = selectedConditions.length === 0 || selectedConditions.includes(m.condition);
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 text-black">
      ...
      {auth.user && (
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <h1 className="text-2xl font-bold text-green-700">
            Browse Materials{" "}
            <span className="text-xs text-gray-400 font-normal">{">"} Donate Materials</span>
          </h1>
      ...
      <div className="flex flex-col md:flex-row gap-4 mt-4 text-gray-600">
        {/* Sidebar / Collapsible */}
        <div className="w-full md:w-1/5 bg-gray-100 p-4 rounded-lg">
          {/* Toggle Button (for mobile) */}
          <button
            onClick={() => setShowFilters(!showFilters)}
            className="w-full flex items-center justify-between px-3 py-2 bg-gray-200 rounded-md hover:bg-gray-300 md:hidden"
          >
